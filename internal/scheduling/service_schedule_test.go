...
func TestGenerateScheduleSlots_Types(t *testing.T) {
	...
	// One montage day (18 June only — 19 June is festival day)
	assert.Len(t, montage, 1)
	assert.Equal(t, 8, montage[0].StartTime.In(warsawLocation).Hour())
	assert.Equal(t, 20, montage[0].EndTime.In(warsawLocation).Hour())
	assert.Equal(t, 7.0, montage[0].CreditHours)
	assert.Equal(t, 8, montage[0].Capacity)
	...
	// One demontage day
	assert.Len(t, demontage, 1)
	assert.Equal(t, 7.0, demontage[0].CreditHours)
	assert.Equal(t, 3, demontage[0].Capacity)
...
func TestGenerateScheduleSlots_MultipleMontageDays(t *testing.T) {
	...
	// 16, 17, 18 June = 3 days
	assert.Len(t, montage, 3)
}
